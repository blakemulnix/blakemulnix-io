...
const collections = named
  .map(({ id, title }) => ({
    id,
    title,
    slugs: ordered.filter((photo) => photo.collection === id).map((photo) => photo.slug),
  }))
  .filter((collection) => collection.slugs.length > 0)

const unfiled = ordered.filter((photo) => !named.some((collection) => collection.id === photo.collection))
if (unfiled.length) {
  console.warn(`  ${unfiled.length} photos are in no collection and will not appear on the wall`)
}
...
console.log(`wrote ${GENERATED_COLLECTIONS} (${collections.length} collections)`)
